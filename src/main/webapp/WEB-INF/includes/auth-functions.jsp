<%-- auth-functions.jsp - Funciones JavaScript reutilizables --%>
<script>
// Detectar API URL
        ...
        if (user) {
            navButtons.innerHTML = `
                <div class="flex items-center space-x-4">
                    ${user.role === 'ADMIN' ? '<a href="admin.jsp" class="text-gray-700 hover:text-purple-600 transition"><i class="fas fa-cog mr-1"></i> Admin</a>' : ''}
...
async function handleLogin(e) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.role === 'ADMIN') {
                setTimeout(() => window.location.href = 'admin.jsp', 1000);
            }
        } else {
            showToast('Error de autenticación', data.message || 'Credenciales incorrectas', 'error');
        }
    } catch (error) {
...
async function handleRegister(e) {
    ...
    try {
        ...
        if (response.ok) {
            showToast('¡Cuenta creada!', `Bienvenido ${data.username}. Ahora inicia sesión`, 'success');
            openAuthModal('login');
        } else {
